...
router.get('/viewApplicantinfo/:idno', (req, res) => {
   ...
   datb.query('SELECT * FROM applicantinfo WHERE idno = ?',[idno], (error, results,fields) => {
       if(error) throw error;
       res.send({results});
   });
});
...
router.put('/DeclineApplication',(req ,res)=>{
    ...
    datb.query('UPDATE idno  SET appl_status = 0 where idno=  "'+idno+'"',(error,results,fields)=>
   {
     if(error) throw error
  else 
    {
     datb.query('select * from applicantinfo where appl_status = 0',function (error, results, fields){
...
outer.put('/AcceptAppl',(req ,res)=>{

    let idno = req.body.idno
    datb.query('UPDATE applicantinfo  SET appl_status = 1 where idno =  "'+idno+'"',(error,results,fields)=>
    {
   if(error) throw error
  
  else 
  {
  datb.query('select * from applicantinfo where appl_status = 1 ',function (error, results, fields){
      return res.send({results})
  ...
  router.get('/declinedApp',(req,res)=>{
    datb.query('SELECT count(idno) AS totAppl from applicantinfo where appl_status = 0 ',(error,results,fields)=>{
      if(error)throw error
      else{
  ...
  router.get('/AcceptedApp',(req,res)=>{
    datb.query('SELECT count(idno) AS totalApplication from applicantinfo where appl_status = 1 ',(error,results,fields)=>{
      if(error)throw error
      else{
